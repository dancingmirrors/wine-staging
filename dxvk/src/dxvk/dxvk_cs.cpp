...
namespace dxvk {
  ...
  void DxvkCsChunkPool::freeChunk(DxvkCsChunk* chunk) {
    ...
    // On 32-bit platforms, limit the number of cached chunks
    // to reduce memory pressure from the chunk pool
    constexpr size_t maxCachedChunks = env::is32BitHostPlatform() ? 4 : 16;
