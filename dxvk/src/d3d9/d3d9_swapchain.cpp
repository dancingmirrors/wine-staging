...
namespace dxvk {
  ...
  void D3D9SwapChainEx::RecreateSwapChain(BOOL Vsync) {
    ...
    // Skip recreation if the swap chain is still compatible.
    // This reduces expensive GPU synchronization during window
    // resizing by accepting suboptimal swapchains when possible.
    if (m_presenter->hasSwapChain()) {
      vk::PresenterInfo info = m_presenter->info();

      // Check if present mode is acceptable
      bool modeMatch = false;
      for (uint32_t i = 0; i < presenterDesc.numPresentModes && !modeMatch; i++)
        modeMatch = info.presentMode == presenterDesc.presentModes[i];

      // Check if format is acceptable
      bool formatMatch = false;
      for (uint32_t i = 0; i < presenterDesc.numFormats && !formatMatch; i++)
        formatMatch = info.format.format     == presenterDesc.formats[i].format
                   && info.format.colorSpace == presenterDesc.formats[i].colorSpace;

      // Skip recreation if mode and format match - size differences
      // are handled by the compositor/blitter (suboptimal but functional)
      if (modeMatch && formatMatch)
        return;
    }
