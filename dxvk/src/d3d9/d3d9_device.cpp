...
namespace dxvk {
  ...
  D3D9DeviceEx::D3D9DeviceEx(
    ...
    m_cursor.setForceHide(m_d3d9Options.forceHideHardwareCursor);
  }
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::Clear(
    ...
    if (!Count) {
      ...
      for (uint32_t i = 0; i < Count; i++) {
        ...
        if (std::min<uint32_t>(pRects[i].x2, offset.x + extent.width) <= uint32_t(rectOffset.x)
          || std::min<uint32_t>(pRects[i].y2, offset.y + extent.height) <= uint32_t(rectOffset.y)) {
          continue;
        }
    ...
    if (likely(changed)) {
      ...
      switch (State) {
        ...
        case D3DRS_DITHERENABLE:
          // Dithering is not implemented, but we store the state
          // to avoid the unhandled render state warning.
          break;
