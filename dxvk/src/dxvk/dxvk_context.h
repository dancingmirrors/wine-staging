...
namespace dxvk {
  ...
  class DxvkContext : public RcObject {
    // Use smaller staging buffer on 32-bit platforms to reduce memory pressure
    constexpr static VkDeviceSize StagingBufferSize = env::is32BitHostPlatform() ? (1ull << 20) : (4ull << 20);
  public:
